...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/assetsadapterstore/iqchain-adapter/iqchain_txsigner"
...
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//sig, err := iqchain_txsigner.Default.SignTransactionHash(msg, keyBytes, decoder.wm.Config.CurveType)
			//
			//if err != nil {
			//
...
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			realSig := iqchain_txsigner.Default.SignSerialize(sig)
